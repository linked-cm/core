...
- **Context unified on `{$ctx}` — DONE (real positions) + inbound-decode (interop).** Introduced
  ...
  - **Builder-side mutation field-values — FIXED in review (see Review §Gap 1).** The factory now
    *preserves* a live `PendingQueryContext` through normalization (instead of collapsing it to
    ...
    pre-pass in `lower()`.
  - **Where-clause args — FIXED in review (see Review §Where-context).** A context in a `.where()`
...
### Where-context (post-review follow-up — the last deferred context position)

Closed the where-clause-arg context gap, completing `{$ctx}` across **all** positions (subject,
target, mutation field-value, where-arg). Confirmed scope with the user: **full unification**, and a
SELECT with an unresolved where-context **short-circuits to null** (mirroring the pending-subject
behavior).

- Where-clause context now carries the context **name** through the expression IR
  (`reference_expr.contextName` / `context_property_expr.contextName`) instead of baking the id at
  `.where()`-build time. `setQueryContext` stamps `__queryContextName`; `ExpressionNode.toIRExpression`
  + `SelectQuery.toExpressionNode`/`.equals` emit the name and no longer throw on an unset context.
- Resolution happens at lowering in `IRLower.lowerWhere` (the shared select+mutation chokepoint):
  `resolveWhereContextRefs` walks the filter tree and resolves each context name to its IRI, throwing
  `UnresolvedContextError` if unset. The **lowered** IR is identical to the old baked-id form, so SPARQL
  and the IR golden tests are unaffected.
- SELECT `exec()` catches `UnresolvedContextError` and returns null (the reactive layer re-runs once
  the context lands); mutations propagate the throw.
- The wire shows the ref inside the expression IR (`{kind:'reference_expr', contextName}`) rather than
  the literal top-level `{$ctx}` marker — the conceptual unification (one name-carried,
